...
if __name__ == "__main__":
    ...
    start = sys.argv[1]
    stop = sys.argv[2]
    p_days = int(sys.argv[3])
    period = int(sys.argv[4])
    dist_period = 0
    na_hist_stocks = {}
    for stock_name in stocks_df["Stock"]:

        stock_is_not_in = len([el for el in os.listdir(plot_dir) if el.startswith(stock_name)]) == 0

        if stock_is_not_in and not(stock_name in na_hist_stocks):
            print("Backtest " + stock_name)
            # Getting Raw data
            rw_data = get_raw_data(stock_name, start, stop)

            if len(rw_data)<calibration_period:
                na_hist_stocks[stock_name] = np.nan
            ...
            if check_unavailable_data(new_ft_data, calibration_period, period):
                na_hist_stocks[stock_name] = np.nan

            if not(stock_name in na_hist_stocks):
